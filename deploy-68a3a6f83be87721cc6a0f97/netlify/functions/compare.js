...
exports.handler = async (event, context) => {
	...
	try {
		...
		const systemPrompt = [
			...
			'- Only provide information you are 100% confident about based on your training data,- Confirm using:Official datasheet (Features, Description, Ordering Information) Distributor listings (e.g., Digi-Key, Mouser)',
			'- Prioritize accuracy over completeness - it is better to provide less information that is correct than more information that may be wrong. Do not assume or invent specs or package type.',
			...
			'- Detailed electrical specifications with exact values. Verify voltage, current, frequency, timing, and power from the datasheet (only if verified)',
			'- Register maps and firmware compatibility analysis (with confidence levels)',
			'- Package and footprint compatibility details. Confirm pinout from datasheet. List a pin by pin comparing each pin number (with verification status)',
